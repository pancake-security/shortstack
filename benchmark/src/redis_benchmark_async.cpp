...
void client(int idx, int client_batch_size, trace_vector &trace, std::string output_path, std::shared_ptr<host_info> hinfo, std::atomic<int> &xput, int queue_depth, std::vector<int> &latencies, std::vector<std::string> &diags, std::shared_ptr<host_info> hosts) {
    std::vector<host> kv_hosts;
    hosts->get_hosts_by_type(HOST_TYPE_KV, kv_hosts); 
      
    redis_interface client(kv_hosts[0].hostname, kv_hosts[0].port, client_batch_size, get_cb, put_cb_dummy);
    ...
    std::string location = output_path + "-redisclient" + std::to_string(idx)+ ".lat";
...
int main(int argc, char *argv[]) {
    ...
    while ((o = getopt(argc, argv, "h:t:n:o:q:gs:")) != -1) {
        switch (o) {
            case 'h':
